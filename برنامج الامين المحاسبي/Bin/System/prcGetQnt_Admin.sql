##################################################
CREATE PROCEDURE prcGetQnt_Admin
	@StartDate 			[DATETIME],
	@EndDate 			[DATETIME],
...
AS
	...
	IF( EXISTS( SELECT * FROM [vwbu] WHERE [buDate] < @StartDate OR [buDate]> @EndDate)
		...
		--OR ISNULL( @SrcTypes, '') <> ''   -- selected src types so from bi, bu ---ask ali
	...
	BEGIN
		...
		BEGIN  ---select *from VwMsSt
			...
			WHERE
				((@StoreGUID = 0x0) 	OR ([VMsSt].[msStorePtr] IN( SELECT [StoreGUID] FROM [#StoreTbl])))
	...
	EXEC prcGetQnt_Admin
	'1/1/2000'--	@StartDate DATETIME,
	,'8/31/2007'--	@EndDate DATETIME,
