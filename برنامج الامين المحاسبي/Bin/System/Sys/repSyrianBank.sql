...
BEGIN 
        ...
        SELECT my.Code,  
				SUM(ISNULL(En.enDebit - En.enCredit, 0)),  
				SUM(ISNULL((En.enDebit - En.enCredit) / ISNULL(En.enCurrencyVal,1), 0)) ,  
                 0, 
                 0 
...
AS
	...
	*/
	...
	SELECT @MinValue = CurrencyVal * 300 FROM my000 WHERE UPPER(code) = 'USD'
...
(
	...
	@GroupBy			INT -- 0 no group, 1 group by month, 2 group by day , 3 group by week
)
	RETURNS @Result TABLE(
						...
						Buy_CASH_Amount_Public FLOAT DEFAULT 0, 
						Buy_CASH_CurrAmount_Public FLOAT DEFAULT 0,  -- ‘—«¡ ‰ﬁœÌ Ã„ÂÊ—
						Buy_CASH_Amount_Bank FLOAT DEFAULT 0, 
						Buy_CASH_CurrAmount_Bank FLOAT DEFAULT 0,  -- ‘—«¡ ‰ﬁœÌ „’«—›
						Buy_CASH_Amount_Exchanger FLOAT DEFAULT 0, 
						Buy_CASH_CurrAmount_Exchanger FLOAT DEFAULT 0,  -- ‘—«¡ ‰ﬁœÌ ’Ì«—›…
						Buy_Check_Amount FLOAT DEFAULT 0, 
						Buy_Check_CurrAmount FLOAT DEFAULT 0, -- ° ‘—«¡ ‘Ìﬂ« 
						Buy_Transfer_Amount FLOAT DEFAULT 0, 
						Buy_Transfer_CurrAmount FLOAT DEFAULT 0, -- ‘—«¡ ÕÊ«·« 
						Sell_CASH_Amount_Public FLOAT DEFAULT 0, 
						Sell_CASH_CurrAmount_Public FLOAT DEFAULT 0,  -- »Ì⁄ ‰ﬁœÌ Ã„ÂÊ—
						Sell_CASH_Amount_Bank FLOAT DEFAULT 0, 
						Sell_CASH_CurrAmount_Bank FLOAT DEFAULT 0,  -- »Ì⁄ ‰ﬁœÌ „’«—›
						Sell_CASH_Amount_Exchanger FLOAT DEFAULT 0, 
						Sell_CASH_CurrAmount_Exchanger FLOAT DEFAULT 0,  -- »Ì⁄ ‰ﬁœÌ ’Ì«—›…
						Sell_Check_Amount FLOAT DEFAULT 0, 
						Sell_Check_CurrAmount FLOAT DEFAULT 0, -- ° »Ì⁄ ‘Ìﬂ« 
						Sell_Transfer_Amount FLOAT DEFAULT 0, 
						Sell_Transfer_CurrAmount FLOAT DEFAULT 0 -- »Ì⁄ ÕÊ«·« 
				)
AS
BEGIN
	...
	SET @currentYear =  CAST(DATEPART(YYYY,  GETDATE()) AS CHAR(4))
	...
	SELECT 
		...
		CASE @GroupBy 
			...
			WHEN 3 THEN CAST (DATEPART(ww, ex.[DATE]) AS NVARCHAR(2))
			ELSE ''
		END,	
		ex.CustomerType,
		IsNull(SUM(ex.CashAmount),0),
		IsNull(SUM((ex.CashAmount / ex.CashCurrencyVal) /*/ 1000*/),0),
		0 -- exchange				 
	...
	SELECT 
		...
		END,
		ex.CustomerType,
		IsNull(SUM(CashDetail.Amount),0),
		IsNull(SUM((CashDetail.Amount / CashDetail.CurrencyVal) /*/ 1000*/),0),
		0 -- exchange				 
	FROM trnexchange000 As ex
	...
	SELECT 
		...
		CASE @GroupBy 
			...
			WHEN 3 THEN CAST (DATEPART(ww, [DATE]) AS NVARCHAR(2))
			ELSE ''
		END,
		0, -- CustomerType
		IsNull(SUM(MustCashedAmount),0),
		IsNull(SUM((MustCashedAmount / CurrencyVal) /*/ 1000*/),0), 
		2
	...
	SELECT 
		...
		END,
		ex.CustomerType,
		ISNull(SUM(ex.PayAmount),0),
		ISNull(SUM((ex.PayAmount / ex.PayCurrencyVal) /*/ 1000*/),0),
		0 -- exchange				 
	FROM trnexchange000 As ex
	WHERE 	
		ex.bSimple = 1 AND
	...
	SELECT 
		...
		END,		
		ex.CustomerType,
		IsNull(SUM(Detail.Amount),0),
		IsNull(SUM((Detail.Amount / Detail.CurrencyVal) /*/ 1000*/),0),
		0 -- exchange				 
	FROM trnexchange000 As ex
	INNER JOIN trnExchangeDetail000 As Detail ON ex.[GUID] = Detail.ExchangeGUID AND Detail.Type = '1'
	WHERE 	
	...
	SELECT 
		...
		DATEPART(m, [DATE]), 
		0, -- CustomerType
		IsNull(SUM(MustPaidAmount),0),
		IsNull(SUM((MustPaidAmount / PayCurrencyVal) /*/ 1000*/),0), 
	...
	IF (@GroupBy = 3)
	BEGIN
		DECLARE @FromDateWeek INT
		SET @FromDateWeek = DATEPART(WEEK, @FROMDate)
		INSERT INTO @Result(CurrencyGUID, WeekNumber)
		SELECT 
			my.[GUID],
			v.Number + @FromDateWeek
		FROM my000 AS my, master..spt_values AS V
		WHERE V.type = 'P' AND v.number <= DATEDIFF(WEEK, @FROMDate, @ToDate)
		END
	ELSE
	-- no group	
	BEGIN
	...
	UPDATE @Result
		SET 		
		-- ‰ﬁœÌ Ã„ÂÊ—
		Buy_CASH_Amount_Public = ISNull(CASE Buy.[Type] 
										WHEN 0 THEN (
													SELECT top 1 AMOUNT FROM @Buy WHERE CustomerType = 0  AND CurrencyGuid = @CurrencyGUID AND (@GroupBy = 0 OR (@GroupBy = 1 AND GroupBy = MonthNumber) OR (@GroupBy = 2 AND GroupBy = CAST ([DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND GroupBy = WeekNumber) )
													) 
										ELSE 0 END,0),
		Buy_CASH_CurrAmount_Public = IsNull(CASE Buy.[Type]
										WHEN 0 THEN (
													SELECT top 1 CurrAmount FROM @Buy WHERE CustomerType = 0 AND CurrencyGuid = @CurrencyGUID AND (@GroupBy = 0 OR (@GroupBy = 1 AND GroupBy = MonthNumber) OR (@GroupBy = 2 AND GroupBy = CAST ([DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND GroupBy = WeekNumber) )
													) 
										ELSE 0 END,0),
		-- ‰ﬁœÌ „’«—›
		Buy_CASH_Amount_Bank = IsNull( CASE Buy.[Type] 
										WHEN 0 THEN (
													SELECT top 1 AMOUNT FROM @Buy WHERE CustomerType = 1  AND CurrencyGuid = @CurrencyGUID AND (@GroupBy = 0 OR (@GroupBy = 1 AND GroupBy = MonthNumber) OR (@GroupBy = 2 AND GroupBy = CAST ([DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND GroupBy = WeekNumber) )
													) 
										ELSE 0 END,0),
		Buy_CASH_CurrAmount_Bank = IsNull(CASE Buy.[Type] 
										WHEN 0 THEN (
													SELECT top 1 CurrAmount FROM @Buy WHERE CustomerType = 1 AND CurrencyGuid = @CurrencyGUID AND (@GroupBy = 0 OR (@GroupBy = 1 AND GroupBy = MonthNumber) OR (@GroupBy = 2 AND GroupBy = CAST ([DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND GroupBy = WeekNumber) )
													) 
										ELSE 0 END,0),
		-- ‰ﬁœÌ ’Ì«—›…
		Buy_CASH_Amount_Exchanger =IsNull( CASE Buy.[Type] 
										WHEN 0 THEN (
													SELECT top 1 Amount FROM @Buy WHERE CustomerType = 2 AND CurrencyGuid = @CurrencyGUID AND (@GroupBy = 0 OR (@GroupBy = 1 AND GroupBy = MonthNumber) OR (@GroupBy = 2 AND GroupBy = CAST ([DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND GroupBy = WeekNumber) )
													) 
										ELSE 0 END,0),
		Buy_CASH_CurrAmount_Exchanger = IsNull(CASE Buy.[Type] 
										WHEN 0 THEN(
													SELECT top 1 CurrAMOUNT FROM @Buy WHERE CustomerType = 2 AND CurrencyGuid = @CurrencyGUID AND (@GroupBy = 0 OR (@GroupBy = 1 AND GroupBy = MonthNumber) OR (@GroupBy = 2 AND GroupBy = CAST ([DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND GroupBy = WeekNumber) )
													) 
										ELSE 0 END ,0),
		-- ‘Ìﬂ«  
		Buy_Check_Amount = IsNull(CASE Buy.[Type] 
									WHEN 1 THEN	
										Buy.Amount 
									ELSE 0 END,0),
		Buy_Check_CurrAmount = IsNull(CASE Buy.[Type] 
									WHEN 1 THEN Buy.CurrAmount 
									ELSE 0 END,0),
		-- ÕÊ«·« 
		Buy_Transfer_Amount = IsNull(CASE Buy.[Type] 
									WHEN 2 THEN Buy.Amount 
									ELSE 0 END,0),
		Buy_Transfer_CurrAmount = IsNull(CASE Buy.[Type]
									WHEN 2 THEN Buy.CurrAmount 
									ELSE 0 END,0)
	FROM @Result AS res
	INNER JOIN @Buy AS Buy ON Buy.CurrencyGUID = res.CurrencyGUID 
			AND (@GroupBy = 0 OR (@GroupBy = 1 AND Buy.GroupBy = res.MonthNumber) OR (@GroupBy = 2 AND Buy.GroupBy = CAST (res.[DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND Buy.GroupBy = res.WeekNumber))
	WHERE res.CurrencyGUID = @CurrencyGUID
	
	UPDATE @Result
		SET 		
		-- ‰ﬁœÌ Ã„ÂÊ—
		Sell_CASH_Amount_Public = IsNull(CASE Sell.[Type] 
											WHEN 0 THEN (
														SELECT top 1 AMOUNT FROM @Sell WHERE CustomerType = 0 AND CurrencyGuid = @CurrencyGUID AND (@GroupBy = 0 OR (@GroupBy = 1 AND GroupBy = MonthNumber) OR (@GroupBy = 2 AND GroupBy = CAST ([DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND GroupBy = WeekNumber) )
														) 
												ELSE 0 END,0),
		Sell_CASH_CurrAmount_Public = IsNull(CASE Sell.[Type] 
											WHEN 0 THEN (
														SELECT top 1  CurrAMOUNT FROM @Sell WHERE CustomerType = 0 AND CurrencyGuid = @CurrencyGUID AND (@GroupBy = 0 OR (@GroupBy = 1 AND GroupBy = MonthNumber) OR (@GroupBy = 2 AND GroupBy = CAST ([DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND GroupBy = WeekNumber) )
														) 
												ELSE 0 END,0),
		-- ‰ﬁœÌ „’«—›
		Sell_CASH_Amount_Bank = IsNull(CASE Sell.[Type] 
											WHEN 0 THEN (
														SELECT top 1 AMOUNT FROM @Sell WHERE CustomerType = 1 AND CurrencyGuid = @CurrencyGUID AND (@GroupBy = 0 OR (@GroupBy = 1 AND GroupBy = MonthNumber) OR (@GroupBy = 2 AND GroupBy = CAST ([DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND GroupBy = WeekNumber) )
														) 
											ELSE 0 END,0),
		Sell_CASH_CurrAmount_Bank = IsNull(CASE Sell.[Type] 
											WHEN 0 THEN (
														SELECT top 1 CurrAmount FROM @Sell WHERE CustomerType = 1 AND CurrencyGuid = @CurrencyGUID AND (@GroupBy = 0 OR (@GroupBy = 1 AND GroupBy = MonthNumber) OR (@GroupBy = 2 AND GroupBy = CAST ([DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND GroupBy = WeekNumber) )
														) 
											ELSE 0 END,0),
		-- ‰ﬁœÌ ’Ì«—›…
		Sell_CASH_Amount_Exchanger = IsNull(CASE Sell.[Type] 
											WHEN 0 THEN (
														SELECT top 1 AMOUNT FROM @Sell WHERE CustomerType = 2 AND CurrencyGuid = @CurrencyGUID AND (@GroupBy = 0 OR (@GroupBy = 1 AND GroupBy = MonthNumber) OR (@GroupBy = 2 AND GroupBy = CAST ([DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND GroupBy = WeekNumber) )
														) 
											ELSE 0 END,0),
		Sell_CASH_CurrAmount_Exchanger = IsNull(CASE Sell.[Type] 
												WHEN 0 THEN(
														SELECT top 1 CurrAMOUNT FROM @Sell WHERE CustomerType = 2 AND CurrencyGuid = @CurrencyGUID AND (@GroupBy = 0 OR (@GroupBy = 1 AND GroupBy = MonthNumber) OR (@GroupBy = 2 AND GroupBy = CAST ([DayDate] AS NVARCHAR(10)))  OR (@GroupBy = 3 AND GroupBy = WeekNumber) )
														) 
												ELSE 0 END,0),
		-- ‘Ìﬂ«  
		Sell_Check_Amount = IsNull(CASE Sell.[Type] 
										WHEN 1 THEN Sell.Amount 
										ELSE 0 END,0),
		Sell_Check_CurrAmount = IsNull(CASE Sell.[Type] 
										WHEN 1 THEN Sell.CurrAmount 
										ELSE 0 END,0),
		-- ÕÊ«·« 
		Sell_Transfer_Amount = ISNULL(CASE Sell.[Type] 
										WHEN 2 THEN Sell.Amount 
										ELSE 0 END,0),
		Sell_Transfer_CurrAmount =ISNull( CASE Sell.[Type] 
											WHEN 2 THEN Sell.CurrAmount 
											ELSE 0 END,0)
	FROM @Result AS res
	INNER JOIN @Sell AS Sell ON Sell.CurrencyGUID = res.CurrencyGUID 
	AND (@GroupBy = 0 OR (@GroupBy = 1 AND Sell.GroupBy = res.MonthNumber) OR (@GroupBy = 2  AND Sell.GroupBy = CAST (res.[DayDate] AS NVARCHAR(10))) OR (@GroupBy = 3 AND Sell.GroupBy = res.WeekNumber))
	WHERE res.CurrencyGUID = @CurrencyGUID
	UPDATE @Result
		SET 
		WeekStartDate =  DATEADD(wk, DATEDIFF(wk, 6, '1/1/' + @currentYear) + (WeekNumber-1), 5)
...
AS
	...
	*/
	SET DATEFIRST 6
	--select *  FROM FnRepSyrianBank11(@FROMDate, @ToDate, @ExchangeForeign, @GroupBy)
	DECLARE @Result TABLE(
						CurrencyGUID UNIQUEIDENTIFIER, 
						[DayDate] DATETIME,
						WeekNumber INT,
	...
	SELECT 
				...
				WeekNumber,
				WeekStartDate,
				Buy_CASH_Amount_Public, 
				Buy_CASH_CurrAmount_Public ,  -- ‘—«¡ ‰ﬁœÌ Ã„ÂÊ—
				Buy_CASH_Amount_Bank, 
				Buy_CASH_CurrAmount_Bank ,  -- ‘—«¡ ‰ﬁœÌ „’«—›
				Buy_CASH_Amount_Exchanger , 
				Buy_CASH_CurrAmount_Exchanger ,  -- ‘—«¡ ‰ﬁœÌ ’Ì«—›…
				Buy_Check_Amount , 
				Buy_Check_CurrAmount , -- ° ‘—«¡ ‘Ìﬂ« 
				Buy_Transfer_Amount , 
				Buy_Transfer_CurrAmount , -- ‘—«¡ ÕÊ«·« 
				Sell_CASH_Amount_Public , 
				Sell_CASH_CurrAmount_Public ,  -- »Ì⁄ ‰ﬁœÌ Ã„ÂÊ—
				Sell_CASH_Amount_Bank , 
				Sell_CASH_CurrAmount_Bank ,  -- »Ì⁄ ‰ﬁœÌ „’«—›
				Sell_CASH_Amount_Exchanger , 
				Sell_CASH_CurrAmount_Exchanger ,  -- »Ì⁄ ‰ﬁœÌ ’Ì«—›…
				Sell_Check_Amount , 
				Sell_Check_CurrAmount, -- ° »Ì⁄ ‘Ìﬂ« 
				Sell_Transfer_Amount , 
				Sell_Transfer_CurrAmount  -- »Ì⁄ ÕÊ«·« 
	 FROM FnRepSyrianBank11(@FROMDate, @ToDate, @ExchangeForeign, @GroupBy)
	...
	SELECT 
			...
			WeekNumber,
			WeekStartDate,
			ISNULL(Buy_CASH_CurrAmount_Public, 0) AS Buy_Cash_Public, -- ‘—«¡ ‰ﬁœÌ Ã„ÂÊ— 
			ISNULL(Buy_CASH_Amount_Public, 0) As Buy_Cash_Curr_Public,
			ISNULL(Buy_CASH_CurrAmount_Bank, 0) AS Buy_Cash_Bank, -- ‘—«¡ ‰ﬁœÌ „’«—›
			ISNULL(Buy_CASH_Amount_Bank, 0) AS Buy_Cash_Curr_Bank,
			ISNULL(Buy_CASH_CurrAmount_Exchanger, 0) AS Buy_Cash_Exchanger, -- ‘—«¡ ‰ﬁœÌ ’Ì«—›…
			ISNULL(Buy_CASH_Amount_Exchanger, 0) AS Buy_Cash_Curr_Exchanger,
			ISNULL(Buy_Check_CurrAmount, 0) AS Buy_Check, --  ‘—«¡ ‘Ìﬂ« 
			ISNULL(Buy_Check_Amount, 0) AS Buy_Curr_Check,
			ISNULL(Buy_Transfer_CurrAmount, 0) AS Buy_Transfer, -- ‘—«¡ ÕÊ«·« 
			ISNULL(Buy_Transfer_Amount, 0) AS Buy_Curr_Transfer,
			ISNULL(Sell_CASH_CurrAmount_Public, 0) AS Sell_Cash_Public,  -- »Ì⁄ ‰ﬁœÌ Ã„ÂÊ—
			ISNULL(Sell_CASH_Amount_Public, 0) AS Sell_Cash_Curr_Public,
			ISNULL(Sell_CASH_CurrAmount_Bank, 0) AS Sell_Cash_Bank,  -- »Ì⁄ ‰ﬁœÌ „’«—›
			ISNULL(Sell_CASH_Amount_Bank, 0) AS Sell_Cash_Curr_Bank, 
			ISNULL(Sell_CASH_CurrAmount_Exchanger, 0) AS Sell_Cash_Exchanger,  -- »Ì⁄ ‰ﬁœÌ ’Ì«—›…
			ISNULL(Sell_CASH_Amount_Exchanger, 0) AS Sell_Cash_Curr_Exchanger,
			ISNULL(Sell_Check_CurrAmount, 0) AS Sell_Check, -- ° »Ì⁄ ‘Ìﬂ« 
			ISNULL(Sell_Check_Amount, 0) AS Sell_Curr_Check,
			ISNULL(Sell_Transfer_CurrAmount, 0) AS Sell_Transfer, -- »Ì⁄ ÕÊ«·« 
			ISNULL(Sell_Transfer_Amount, 0) AS Sell_Curr_Transfer,
			0 AS IsTotal
	FROM @Result AS R
	INNER JOIN MY000 AS M ON M.GUID = CurrencyGUID
	...
	SELECT	
			...
			1 AS IsTotal
	FROM @Result 
	GROUP BY [DayDate], WeekNumber
	
	ORDER BY [DayDate], WeekNumber, IsTotal, CurrencyNumber
##########################################################################
...
AS 
	...
	FROM	
		...
		LEFT JOIN TrnBranch000 AS sourceBR ON sourceBR.GUID = V.TVSourceBranch 
	...
	SELECT
		...
		I.Guid, -- VoucherGuid
		0x, -- VoucherTypeGuid
		TrnNumber, -- VoucherCode
		[Date], -- [Date] 
		0x, -- Sender
		SecondPersonName + ' ' + SecondPersonFather + '' + SecondPersonLastName, -- SenderName
		'', -- SenderPhone
		0x, -- Receiver
		FirstPersonName + ' ' + FirstPersonFather + ' ' + FirstPersonLastName, --ReceiverName
		'', -- ReceiverPhone
		'', --SenderAddress
		DocumentNumber, -- ReceiverIdentityCard
		ValueInCardCurrency * ActualCurrencyValue, -- Amount
		InCardCurrencyID, -- Currency
		ActualCurrencyValue, -- CurrVal 
		Wags, -- NetWages
		ValueInDefaultCurrency, -- PayAmount
		(SELECT TOP 1 GUID FROM my000 WHERE CurrencyVal = 1), -- PayCurrency
		1, -- PayCurVal
		dbo.fnOption_Get('TrnCfg_OMT_CompanyName','8000'), -- OfficeName
		[Source], -- OfficeCity
		'', -- OfficeState
		Note, -- Notes
		TrnCause, -- Reason
		I.[Address], --ActualReceiverAddress
		sourceBR.Name
	FROM
		InCardCompany AS I
		INNER JOIN TrnBranch000 AS sourceBR ON sourceBR.GUID = I.BranchID
	WHERE I.[Date] BETWEEN @StartDate AND @EndDate AND @Local = 0
...
AS 
	...
	BEGIN  
	    ...
	    WHERE 
			v.[TVDate] BETWEEN @StartDate AND @EndDate AND v.TVSourceType = 1 AND v.TvDestinationType = 1 AND (TVState <> 15)--«·ÕÊ«·… ·Ì”  „·€Ì…  
	END  
	...
	SELECT
		...
		[Date], -- [Date] 
		0x, -- Sender
		FirstPersonName + ' ' + FirstPersonFather + ' ' + FirstPersonLastName, -- SenderName
		'', -- SenderPhone
		I.[Address], --SenderAddress
		DocumentNumber, -- SenderIdentityCard
		0x, -- Receiver
		SecondPersonName + ' ' + SecondPersonFather + '' + SecondPersonLastName, --ReceiverName
		'', -- ReceiverPhone
		ValueInCardCurrency * CardCurrencyValue, -- Amount
		(SELECT TOP 1 GUID FROM my000 WHERE CurrencyVal = 1), -- Currency
		1, -- CurrVal 
		Wags, -- NetWages
		ValueInCardCurrency, -- PayAmount
		InCardCurrencyID, -- PayCurrency
		CardCurrencyValue, -- PayCurVal
		dbo.fnOption_Get('TrnCfg_OMT_CompanyName','8000'), -- OfficeName
		[Source], -- OfficeCity
		'', -- OfficeState
		Note, -- Notes
...
AS
	...
	SELECT 
		fn.[GUID], fn.[Code], fn.[Name], fn.[LatinName], fn.[TotalBalance], fn.ParentGUID, ac.Name AS ParentAccountName	
	FROM 
		fnTrnSyBkAccountsBalances(0x0, 0, '', @ToDate) AS fn
		INNER JOIN fnTrnGetSyBkAccountsCodesTable(0) AS fncodes ON fncodes.Data = fn.Code
		INNER JOIN ac000 as ac ON ac.Guid = fn.ParentGUID
	ORDER BY fn.code
#####################################################################
...
AS
	...
	( 
		...
		IsMainAssetsAccount				BIT		DEFAULT 0 -- Â· «·Õ”«» Õ”«» „ÊÃÊœ«  —∆Ì”Ì
	)
	
	INSERT INTO @Result(GUID, [Level], Code, [Name], [TotalBalance], [ResidentBalance],
		[Non_ResidentBalance], [ResidentForeignBalance], [Non_ResidentForeignBalance], IsBankAccount, IsMainAssetsAccount)
	SELECT 
		fn.GUID,
		fn.[Level],
		fn.Code,
		...
		CASE WHEN fn.ParentGUID = @BaseAccount THEN 1 ELSE 0 END
...
AS
	...
	( 
		...
		[Non_ResidentForeignBalance]	[FLOAT] DEFAULT 0, -- —’Ìœ «·√Ã‰»Ì €Ì— «·„ﬁÌ„
		IsBankAccount			BIT	DEFAULT 1, -- Â· «·Õ”«» Õ”«» „’—›Ì	
		IsMainLiabilitiesAccount		BIT	DEFAULT 0 -- Â· «·Õ”«» Õ”«» „ÿ«·Ì» —∆Ì”Ì
	)
	
	INSERT INTO @Result(GUID, [Level], Code, [Name], [TotalBalance], [ResidentBalance],
		[Non_ResidentBalance], [ResidentForeignBalance], [Non_ResidentForeignBalance], IsBankAccount, IsMainLiabilitiesAccount)
	SELECT 
		fn.GUID,
		fn.[Level],
		fn.Code,
...
insert into @result 
      select 
           ...
           (Sell_CASH_Amount_Public + Sell_CASH_Amount_Bank + Sell_CASH_Amount_Exchanger) / 1000 AS SumSellCashSY,
            ...
            0,0,
            --------------------------
            IsNull(fnPrev.Balance,0) + (Buy_Transfer_CurrAmount + Buy_CASH_CurrAmount_Public + Buy_CASH_CurrAmount_Bank + Buy_CASH_CurrAmount_Exchanger) -
           ...
           (IsNull(fnPrev.BalanceSY / 1000, 0 ) + (((Buy_Transfer_Amount + Buy_CASH_Amount_Public + Buy_CASH_Amount_Bank + Buy_CASH_Amount_Exchanger) - 
      ...
      From
			...
			LEFT JOIN   FnGetBeginBalanceForiengCurrencies(@begin,@end) AS fnPrev ON fnPrev.CurrencyGuid = bank11.currencyguid
...
END
####################################################################
CREATE FUNCTION FnGetBeginBalanceForiengCurrencies(@Begin DATETIME, @End DATETIME)
	RETURNS @Result Table 
		( 
			CurrencyGuid UNIQUEIDENTIFIER,
			Balance FLOAT,
...
AS 
	...
	SET @FirstPeriodDate = [dbo].[fnDate_Amn2Sql]([dbo].[fnOption_get]('AmnCfg_FPDate', default))
	...
	SELECT ac1.acGUID ,
			...
			ac1.acCurrencyPtr,
			ac1.acParent 
		FROM vwac ac INNER JOIN  
			(
			 SELECT groupAcc.Name AS GroupCurrencyAccName,
					groupAcc.Guid AS GroupCurrencyAccGUID,
					groupAcc.AccountGUID as AccountGuid 
				FROM  
					TrnGroupCurrencyAccount000 groupAcc, vwTrnExchangeTypes ExchangeTypes 
				WHERE 
					(groupAcc.GUID  = ExchangeTypes.GroupCurrencyAccGUID)
					OR 
					(groupAcc.Guid = ExchangeTypes.MngerGroupCurrencyAccGUID)
					OR 
					(groupAcc.Guid = (select  CAST(VALUE AS [UNIQUEIDENTIFIER]) 
			FROM 
				OP000 WHERE NAME  = 'TrnCfg_CurrencyAccount'))
			GROUP BY
			  groupAcc.Name,groupAcc.Guid, groupAcc.AccountGUID
			) AS test 
				 ON  test.AccountGuid = ac.acGuid 
			INNER JOIN vwac ac1 ON ac1.acParent = test.AccountGuid
  
	
	DECLARE @Curr TABLE( DATE SMALLDATETIME,VAL FLOAT, CurrencyPtr UNIQUEIDENTIFIER) 

		INSERT INTO @Curr  
		SELECT DATE,CurrencyVal,temp.acCurrencyPtr   FROM mh000 mh
		INNER JOIN  @Acc temp 
		ON temp.acCurrencyPtr = mh.CurrencyGuid 
		UNION ALL  
		SELECT  '1/1/1980',CurrencyVal,temp.acCurrencyPtr AS CurrencyPtr FROM MY000 my INNER JOIN  @Acc temp 
		ON temp.acCurrencyPtr = my.Guid 
 
 
	  INSERT INTO @BeginResult
			SELECT  test.CurrencyGuid , test.Balance, test.BalanceSy
			FROM 
			(
			SELECT tb.acCurrencyPtr AS CurrencyGuid ,  SUM(AccCurrTb.Balance) AS Balance  , SUM(AccCurrTb.BalanceSy) AS BalanceSy

			FROM (
			 SELECT   
				tbl.acName AS acName,  
				[AC].[acGUID] AS aGuid,      
				[AC].[acParent] AS acParentGuid, 
				tbl.acCurrencyPtr AS CurrencyGuid,     
				SUM( ISNULL([CE].[enDebit] * FACTOR,0)) - SUM( ISNULL([CE].[enCredit] * FACTOR,0)) AS Balance,   
				SUM( ISNULL([CE].[enDebit]  ,0)) - SUM( ISNULL([CE].[enCredit] ,0)) AS BalanceSy
		FROM  (SELECT * ,1 / CASE WHEN enCurrencyPtr = tb2.acCurrencyPtr  THEN 
						[enCurrencyVal] 
					ELSE (SELECT TOP 1 VAL 
					FROM @Curr Curr
					WHERE tb2.acCurrencyPtr = Curr.CurrencyPtr                       
					ANd [DATE] <=  endate  ORDER BY DATE DESC) END FACTOR 
					FROM vwceen e inner join @Acc tb2
					ON e.enAccount = tb2.acGuid) AS [CE]  
			INNER JOIN  @Acc tbl on ce.enAccount = tbl.acGuid
			INNER JOIN vwac AC ON Ac.acGuid = tbl.acGuid 
		WHERE      
				CE.enDate BETWEEN  @FirstPeriodDate AND  DATEADD(ss,-1,@Begin)
		GROUP BY tbl.acName , AC.acGuid, AC.acParent, tbl.acCurrencyPtr			
	   )as AccCurrTb INNER JOIN @Acc tb ON tb.acCurrencyPtr = AccCurrTb.CurrencyGuid
	  GROUP BY tb.acCurrencyPtr,tb.acGUID
	  )as test 
	  GROUP BY test.CurrencyGuid, test.Balance, test.BalanceSy


			
	  INSERT INTO @EndResult
			SELECT  test.CurrencyGuid ,test.EndBalance,test.EndBalanceSy 
			FROM
		  (
			SELECT tb.acCurrencyPtr AS CurrencyGuid 
			 ,SUM(AccCurrTb.EndBalance) AS EndBalance  , SUM(AccCurrTb.EndBalanceSy) AS EndBalanceSy
			FROM (
			 SELECT   
				tbl.acName AS acName,  
				[AC].[acGUID] AS aGuid,      
				[AC].[acParent] AS acParentGuid, 
				tbl.acCurrencyPtr AS CurrencyGuid,     
				 SUM( ISNULL([CE].[enDebit] * FACTOR,0)) - SUM( ISNULL([CE].[enCredit] * FACTOR,0)) AS EndBalance,   
				SUM( ISNULL([CE].[enDebit]  ,0)) - SUM( ISNULL([CE].[enCredit] ,0)) AS EndBalanceSy
		FROM  (SELECT * ,1 / CASE WHEN enCurrencyPtr = tb2.acCurrencyPtr  THEN 
						[enCurrencyVal] 
					ELSE (SELECT TOP 1 VAL 
					FROM @Curr Curr
			...
			INNER JOIN  @Acc tbl on ce.enAccount = tbl.acGuid 
			INNER JOIN vwac AC ON Ac.acGuid = tbl.acGuid 
		WHERE      CE.enDate BETWEEN @FirstPeriodDate AND  @End
	   GROUP BY tbl.acName , AC.acGuid, AC.acParent, tbl.acCurrencyPtr			
	   )as AccCurrTb INNER JOIN @Acc tb ON tb.acCurrencyPtr = AccCurrTb.CurrencyGuid
	  GROUP BY tb.acCurrencyPtr,tb.acGUID
	  )as test 
	  GROUP BY test.CurrencyGuid,test.EndBalance,test.EndBalanceSy 



	  INSERT INTO @Result 
	  SELECT bResult.CurrencyGuid , bResult.Blalance AS Balance , bResult.BalanceSY AS BalanceSY
	  , eResult.EndBlalance AS EndBalance, eResult.EndBalanceSY As EndBalanceSY
	  FROM @BeginResult bResult ,@EndResult eResult 
	  WHERE bResult.CurrencyGuid  = eResult.CurrencyGuid
...
AS
	...
	SELECT 
		fnCurrent.curguid,
		fnCurrent.accguid,
		fnPrev.Balance AS BeginBalance,
		fnPrev.BalanceSY AS BeginBalanceSY,
	...
	FROM
	FnSyrianBank12('1900',@date2) AS fnOld 
			INNER JOIN  FnSyrianBank12(@begin,@end) AS fnCurrent ON fnOld.curguid = fnCurrent.curguid
			LEFT JOIN   FnGetBeginBalanceForiengCurrencies(@begin,@end) AS fnPrev ON fnPrev.CurrencyGuid = fnCurrent.curguid
	Where fnCurrent.curguid <> @BaseCurrency
#####################################################################
