##################################################################################
CREATE PROCEDURE prcShortTermSolvencyMeasures
(
...
AS
       ...
       SELECT * FROM
       (
              SELECT IncomeType Classification, [Date], CASE IncomeType WHEN 18 THEN -1 * SUM(Balance) ELSE SUM(Balance) END Balance
              ...
              WHERE IncomeType IN (15, 16, 18) AND DATE BETWEEN @FromYear AND @ToYear
              GROUP BY IncomeType, Date
       ) T
       --------------------------------------
       UPDATE #result SET Classification = 
               CASE WHEN Classification = 15 THEN 0
					WHEN Classification = 16 THEN 1
	...
	---------------------------------------------------
       ...
       SELECT * FROM
       (
              SELECT 2 Class, [Date], SUM(Balance) Balance
              FROM FABalanceSheetAccount000 AC
              INNER JOIN FABalanceSheetAccountBalance000 BAL ON BAL.AccountGuid = AC.AccountGUID
       ...
       (
              ...
              FROM #result
              WHERE Classification BETWEEN 0 AND 1
              GROUP BY Date
       ) T
	   ---------------------------------------------------
	   INSERT INTO #result (Classification, Date, Balance)
	  ...
	  -------------------------------------------
	   ...
	   ------------------------------------------------------------------
		...
		SELECT DATE ChartDate, CASE ISNULL([4], 0) WHEN 0 THEN 0 ELSE ISNULL([3], 0) / ISNULL([4], 1) END CurrentRatio,
			CASE ISNULL([4], 0) WHEN 0 THEN 0 ELSE (ISNULL([3], 0) - ISNULL([2], 0)) / ISNULL([4], 1) END QuickRatio,
			CASE ISNULL([4], 0) WHEN 0 THEN 0 ELSE ISNULL([1], 0) / ISNULL([4], 1) END CashRatio
		FROM
		(
