...
AS
	...
	FROM  
		[#Result] AS [Gr] INNER JOIN [TypesGroupRepSrcs000] AS [ty] ON [Gr].[GUID] = [ty].[Guid]
		INNER JOIN vwEt VET ON VET.etGUID = TY.TypeGuid 
		INNER JOIN et000 ET ON ET.GUID = VET.etGUID
	...
	FROM  
		[#Result] AS [Gr] INNER JOIN [TypesGroupRepSrcs000] AS [ty] ON [Gr].[GUID] = [ty].[Guid]
		INNER JOIN vwNt VNT ON VNT.ntGUID = TY.TypeGuid 
		INNER JOIN nt000 NT ON NT.GUID = VNT.ntGUID
