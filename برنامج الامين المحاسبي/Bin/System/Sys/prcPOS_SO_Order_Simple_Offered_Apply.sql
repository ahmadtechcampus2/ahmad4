################################################################################
CREATE PROC prcPOS_SO_Order_Simple_Offered_Apply
...
AS 
	...
	OPEN @Items FETCH NEXT FROM @Items INTO @ItemGUID, @ItemMatGUID, @ItemGroupGUID, @ItemQty, @ItemUnitFact, @SO_UnitFact, @ItemBillGUID, @ItemCompositionParentGuid
	...
	BEGIN 
		...
		SET @SO_Qty2 = @SO_Qty * @SO_UnitFact

		SET @IsApplyed = 0

		IF @SO_MatGUID != 0x0 AND (@ItemMatGUID = @SO_MatGUID OR (ISNULL(@ItemCompositionParentGuid,0x0) <> 0x0 AND @ItemCompositionParentGuid = @SO_MatGUID)) AND @ItemQty >= @SO_Qty2
			SET @IsApplyed = 1
		ELSE IF @SO_GroupGUID != 0x0 AND @ItemQty >= @SO_Qty2
		BEGIN
		...
		BEGIN 
			...
			EXEC prcPOS_SO_AddOrderOfferedItems 
					...
					@SO_Qty2, @SO_ApplyOnce, @ItemSOGroup, @ParentItemGUID, @IsReturned
		END		 
		FETCH NEXT FROM @Items INTO @ItemGUID, @ItemMatGUID, @ItemGroupGUID, @ItemQty, @ItemUnitFact, @SO_UnitFact, @ItemBillGUID, @ItemCompositionParentGuid
