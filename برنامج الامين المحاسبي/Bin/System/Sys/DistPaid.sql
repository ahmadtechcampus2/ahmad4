    ...
    BEGIN  
       ...
       ----- From Fast Paid  
		...
		IF @CompareGiftsAndDiscSum = 1	 
			SELECT  
				@CalcDifference =  
					abs(SUM(CASE LineType WHEN 1 THEN NetLoadValue		ELSE NetLoadValue *-1	END)) +  
					abs(SUM(CASE LineType WHEN 1 THEN BonusValue		ELSE BonusValue *-1		END)) +  
					abs(SUM(CASE LineType WHEN 1 THEN DiscountValue		ELSE DiscountValue *-1	END)) +  
					abs(SUM(CASE LineType WHEN 1 THEN DebtsGranted		ELSE DebtsGranted *-1	END)) +  
					abs(SUM(CASE LineType WHEN 1 THEN DebtsCollected	ELSE DebtsCollected *-1	END)) +	 
					abs(SUM(CASE LineType WHEN 1 THEN PaidValue			ELSE PaidValue *-1		END)) 
			FROM  
				#Results1
		ELSE
			BEGIN
