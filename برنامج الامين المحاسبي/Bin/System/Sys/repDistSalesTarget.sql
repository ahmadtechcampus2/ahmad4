...
-- Select * from #aaa
	...
	UPDATE [d] SET	[HiGuid] = [HierarchyGUID], 
					[SalesManGuid] = CASE [CurrSaleMan] WHEN 0 THEN [PrimSalesmanGUID] ELSE [AssisSalesmanGUID] END,
					[CostSalesManGuid] = [sm].[CostGUID],
					[SalesManSecrity] = [sm].[Security]  
	FROM 
		[#Dists] AS [d] 
		INNER JOIN [Distributor000]	 AS [ds] ON [d].[DistGuid] = [ds].[GUID]
		INNER JOIN [DistSalesman000] AS [sm] ON [sm].[Guid] = CASE [CurrSaleMan] WHEN 0 THEN [PrimSalesmanGUID] ELSE [AssisSalesmanGUID] END
	---- Dists HierarchyList
	...
	BEGIN
		...
		FROM 
			#MatTargets AS mt
			INNER JOIN (SELECT	[mt].[MatGuid],
								ISNULL([tr].[Qty], 0) AS TotalTarget,
						...
						FROM #BillTbl AS bt 
						LEFT JOIN vwbubi AS bi ON [bi].[buDate] BETWEEN @PeriodStartDate AND @EndDate 
												AND bi.buCostPtr IN ( select CostSalesManGuid from #Dists )
												AND bt.Type = bi.buType
						INNER JOIN #MatTargets AS mt on mt.MatGuid = bi.biMatPtr
						LEFT JOIN (SELECT
											MatGuid,
											--DistGuid,
											SUM(cmt.CustTarget) AS Qty
										FROM
											distcustmattarget000 AS cmt
											INNER JOIN distcusttarget000 AS ct ON cmt.CustGuid = ct.CustGuid
										WHERE cmt.PeriodGuid = @PeriodGuid AND DistGuid IN (SELECT DistGuid FROM #DistTable)
										GROUP BY
											--DistGuid,
											MatGuid) AS tr ON [tr].[MatGuid] = [mt].[MatGuid]
			...
			--LEFT JOIN #BillTbl AS bt ON bt.Type = bi.buType
						WHERE [mt].[Type] = 1
	...
	BEGIN
		...
		UPDATE #MatTargets 
		SET [Target] = ISNULL([s].[TotalTarget], 0)
		FROM #MatTargets AS mt
							 LEFT JOIN (SELECT
											MatGuid,
											DistGuid,
											SUM(cmt.CustTarget) AS TotalTarget
										FROM
											distcustmattarget000 AS cmt
											INNER JOIN distcusttarget000 AS ct ON cmt.CustGuid = ct.CustGuid
										WHERE cmt.PeriodGuid = @PeriodGuid AND DistGuid IN (SELECT DistGuid FROM #DistTable)
										GROUP BY
											DistGuid,
											MatGuid) AS S ON [S].[MatGuid] = [mt].[MatGuid] AND S.DistGuid = mt.DistGuid
		WHERE [Type] = 2
	...
	IF @ShowCalcTarget = 1
	BEGIN
		SELECT DistGUID, dg.MatGUID, SUM(dq.Qty) AS Qty INTO #DistTargets FROM DistTargetByGroupOrDistributorQty000 AS dq
		INNER JOIN DistTargetByGroupOrDistributorDetails000 AS dd ON dq.ParentGUID = dd.Guid
		INNER JOIN DistTargetByGroupOrDistributor000 AS d ON dd.ParentGUID = d.Guid
		INNER JOIN DisGeneralTarget000 AS dg ON dd.MatGeneralTargetGUID = dg.GUID
		INNER JOIN mt000 AS m On m.GUID = dg.MatGUID
		WHERE dq.DistGUID != 0x AND d.PeriodGuid = @PeriodGuid
		GROUP BY DistGUID ,dg.MatGUID

		CREATE TABLE [#DistResult] (
			[Guid] [UNIQUEIDENTIFIER],
			[HiLevel]		[INT],
			[TypeLevel]		[INT],
			[GroupLevel]	[INT]
		...
		DECLARE @Dist_Guid UNIQUEIDENTIFIER, @Group_Guid UNIQUEIDENTIFIER, @HiLevel INT = 1, @GrLevel INT = 1
		DECLARE vendor_cursor CURSOR  
		FOR SELECT GUID FROM Distributor000 ORDER BY Number
		OPEN vendor_cursor  
		...
		BEGIN 
		BEGIN TRAN T2;
		INSERT INTO #DistResult VALUES(@Dist_Guid,@HiLevel,2,0 )
		INSERT INTO #DistResult SELECT GUID,@HiLevel,1,0 FROM DistHi000 WHERE GUID IN (SELECT HierarchyGUID FROM Distributor000 WHERE Guid = @Dist_Guid)
		IF EXISTS (SELECT MatGUID, @HiLevel, 4, @GrLevel FROM #DistTargets WHERE DistGUID = @Dist_Guid)
		COMMIT TRAN T2;
		ELSE
		ROLLBACK TRAN T2;
		DECLARE mat_cursor CURSOR FOR
		SELECT GUID FROM gr000 WHERE GUID IN (SELECT GroupGUID FROM mt000 WHERE GUID IN (SELECT GUID FROM #DistTargets WHERE DistGUID = @Dist_Guid))
		OPEN mat_cursor  
		...
		BEGIN 
		BEGIN TRAN T1;
		INSERT INTO #DistResult SELECT @Group_Guid, @HiLevel, 3, @GrLevel 
		INSERT INTO #DistResult SELECT MatGUID, @HiLevel, 4, @GrLevel FROM #DistTargets WHERE DistGUID = @Dist_Guid AND MatGUID IN (SELECT GUID FROM mt000 WHERE GroupGUID = @Group_Guid)
		IF EXISTS (SELECT MatGUID, @HiLevel, 4, @GrLevel FROM #DistTargets WHERE DistGUID = @Dist_Guid AND MatGUID IN (SELECT GUID FROM mt000 WHERE GroupGUID = @Group_Guid))
		COMMIT TRAN T1;
		ELSE
		ROLLBACK TRAN T1;
		SET @GrLevel = (@GrLevel + 1)
		FETCH NEXT FROM mat_cursor
		INTO @Group_Guid
		END
		CLOSE mat_cursor
		...
		SELECT 
			...
			SUM([Sales]) AS Sales,
			SUM([Bonus]) AS Bonus,
			SUM([CustsCnt]) AS CustsCnt,
			SUM([BillsCnt]) AS BillsCnt,
			SUM([TargetPercent]) AS TargetPercent,
			SUM([SalesExpected]) AS SalesExpected,
			[Flag] INTO #Result2
		FROM #Result
		GROUP BY
			[Guid],
		...
		SELECT 
			...
			[Flag]
		FROM #Result2 AS r
		INNER JOIN #DistResult AS dr ON r.Guid = dr.Guid
			WHERE 
				(
				([Flag] = 2 					/*Show Dists*/	)	OR
				([Flag] = 1 AND @ShowDistHi = 1	/*Show DistHi*/	)	OR
				([Flag] = 3 AND @ShowGroups = 1	/*Show Groups*/	)	OR
				([Flag] = 4 AND @ShowMats = 1	/*Show Mats*/	))
				ORDER BY dr.HiLevel, dr.GroupLevel,dr.TypeLevel 

	SELECT @RepDaysNum AS RepDaysNum, @PeriodDaysNum AS PeriodDaysNum, @RepDaysNum / @PeriodDaysNum * 100 AS TargetExpectedPercent
	SELECT * FROM #DistTargets
	END
