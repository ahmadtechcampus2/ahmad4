####################################################################################
CREATE PROCEDURE repApprovalsMoveByDate
	@StartDate					DATE,
	@EndDate					DATE,
...
AS
	...
	-------------------------------------------------------------------  
	-------------------------   #CustTbl   ---------------------------   
	-- جدول الزبائن التي تحقق الشروط   
	CREATE TABLE #CustTbl (   
    	CustGuid UNIQUEIDENTIFIER,    
		Security INT)   
	INSERT INTO #CustTbl EXEC prcGetCustsList @CustomerGuid, @AccountGuid, 0x0
	...
	-------------------------   #OrdersCondTbl   ----------------------  
	--  جدول الطلبيات التي تحقق الشروط   
	...
	BEGIN 
		...
		FROM 
			...
			INNER JOIN OrderApprovalStates000 oas ON oa.GUID = oas.ParentGUID
			INNER JOIN us000 us ON us.GUID = oas.UserGUID
		WHERE
			((@UserGuid = 0x0) OR (oas.UserGuid = @UserGuid))
		ORDER BY 
			oa.Number, oas.Number
		...
		FROM
			MgrApp000 mapp
			INNER JOIN OrderApprovalStates000 oas ON mapp.GUID = oas.ParentGUID
			INNER JOIN us000 us ON us.[GUID] = oas.UserGuid
		WHERE
			((@UserGuid = 0x0) OR (oas.UserGuid = @UserGuid))
	END ELSE BEGIN 
		...
		FROM 
			...
			INNER JOIN us000 us ON us.GUID = oas.UserGUID
		WHERE
			((@UserGuid = 0x0) OR (oas.UserGuid = @UserGuid)) AND oas.IsApproved = 1
		ORDER BY 
			oa.Number, oas.Number
	...
	SELECT DISTINCT
		...
		(bt.[Abbrev] + ' - ' + CONVERT(NVARCHAR(10), bu.[buNumber])) AS OrderFormatted,
		bu.[buCust_Name] AS CustomerName,
		st.[Name] AS StoreName,
		ISNULL(co.[Name], '') AS CostCenterName,
		(CASE 
	...
	FROM 
		vwBu bu
		INNER JOIN #RESULT_2 rslt2 ON rslt2.OrderGuid = bu.buGUID
		INNER JOIN bt000 bt ON bt.[GUID] = bu.buType
		INNER JOIN ORADDINFO000 oinf ON oinf.[ParentGuid] = bu.buGUID
		INNER JOIN st000 st ON st.[GUID] = bu.buStorePtr
		INNER JOIN @StoreTbl st2 ON st2.[GUID] = bu.buStorePtr
		INNER JOIN RepSrcs rs ON [IdType] = bu.buType
		LEFT JOIN co000 co ON co.[GUID] = bu.buCostPtr
		INNER JOIN [#CostTbl] co2 ON co2.CostGuid = bu.buCostPtr
		INNER JOIN #CustTbl AS cu ON cu.CustGuid = bu.buCustPtr
		LEFT  JOIN ##CustFlds  C ON C.CustFldGuid  = bu.buCustPtr  
		INNER JOIN ##OrderFlds O ON O.OrderFldGuid = bu.BuGuid
		INNER JOIN #OrdersCondTbl oc ON oc.OrderGuid = bu.buGUID
	WHERE
		bu.buDate BETWEEN @StartDate AND @EndDate
		AND
		((@OrderNumber = 0) OR (bu.[buNumber] = @OrderNumber))
		AND
		(rs.IdTbl = @OrderTypesSrc)
		AND 
		(
			--INCLUDE ACTIVE ORDERS
			((@ShowActiveOrders = 1) AND (oinf.Finished = 0) AND (oinf.Add1 = 0))
			OR 
			--INCLUDE FINISHED ORDERS
			((@ShowFinished = 1) AND ((oinf.Finished = 1) OR (oinf.Finished = 2))) 
			OR 
			--INCLUDE CANCELLED ORDERS
			((@ShowCancelled = 1) AND (oinf.Add1 = 1))
		)
	...
	-- BEGIN
		...
		BEGIN	
			...
			FROM
				#RESULT_2 rs2
				INNER JOIN #result1 rs1 ON rs1.OrderGuid = rs2.OrderGuid
			ORDER BY
				rs1.OrderDate, rs1.OrderGuid, rs1.CustomerName,
				rs2.ApprovalDate
		END
		ELSE
		BEGIN
			...
			FROM 
				#result1
			ORDER BY
				CustomerName, OrderDate, OrderGuid
			...
			FROM
				...
				INNER JOIN #result1 rs1 ON rs1.OrderGuid = rs2.OrderGuid
			ORDER BY
				rs1.CustomerName, rs1.OrderDate, rs1.OrderGuid,
				rs2.ApprovalDate
		END
		*/
####################################################################################
