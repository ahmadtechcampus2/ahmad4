...
CREATE PROCEDURE repReadyMaterialsCostTuning
	( 
		@CostAccountGuid					UNIQUEIDENTIFIER = 0x0
	...
	DECLARE @CostAccountBalance FLOAT	
	SELECT @CostAccountBalance = SUM(En.Debit) - SUM(En.Credit)
	FROM En000 En
	WHERE En.AccountGuid IN ( SELECT Guid FROM dbo.fnGetAccountsList(@CostAccountGuid,0) )
		  AND ( ISNULL(@CostCenter, 0x0) = 0x0 OR En.CostGuid IN ( SELECT Guid FROM dbo.fnGetCostsList(@CostCenter)) )
		  AND En.Date >= @FromDate 
		  AND En.Date <= @ToDate
	
	DECLARE @InsertionAccountBalance FLOAT	
	SELECT @InsertionAccountBalance = SUM(En.Debit) - SUM(En.Credit)
	FROM En000 En
	WHERE En.AccountGuid IN ( SELECT Guid FROM dbo.fnGetAccountsList(@InsertionAccountGuid,0) )
		  AND ( ISNULL(En.CostGuid, 0x0) = 0x0 OR En.CostGuid IN ( SELECT Guid FROM dbo.fnGetCostsList(@CostCenter)) )
		  AND En.Date >= @FromDate 
		  AND En.Date <= @ToDate
	...
	SELECT 
		...
		,@CostAccountBalance CostAccountBalance
		,@InsertionAccountBalance InsertionAccountBalance
		,Stock.Qty Stock
	FROM Bu000 Bu
	...
	WHERE BU.isposted = 1 
		  ...
		  AND Bu.Date <= @ToDate
		  AND ( ISNULL(@CostCenter, 0x0) = 0x0 OR Bi.CostGuid IN ( SELECT Guid FROM dbo.fnGetCostsList(@CostCenter)) ) 
	GROUP BY 
		Mt.Guid
		,Mt.Name
		,Mt.Code
...
INSERT INTO en000     
            ([number], [accountGUID], [Date], [Debit], [Credit], [Notes], [CurrencyGUID],[CurrencyVal], [ParentGUID], [ContraAccGUID])                 
SELECT  
                                    1                             number		 ,        
...
INSERT INTO en000     
            ([number], [accountGUID], [Date], [Debit], [Credit], [Notes], [CurrencyGUID],[CurrencyVal], [ParentGUID], [ContraAccGUID],[CostGuid])                 
SELECT  
                                    1                               number		 ,        
                                    hsh.AccountGuid					accountGUID	 ,       
                                    @Dt								Date		 ,        
