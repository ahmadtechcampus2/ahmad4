########################################################################################
CREATE  FUNCTION  fnGetMatCollectedFieldNameAndGroup (@Colect INT,@Prefix NVARCHAR(10) = '') 
...
AS      
	...
	-------Bill Resource ---------------------------------------------------------           
	CREATE TABLE #Src ( Type UNIQUEIDENTIFIER, Sec INT,ReadPrice INT, UnPostedSec INT , OrderName NVARCHAR(15) COLLATE ARABIC_CI_AI default '',OrderLatinName NVARCHAR(15) COLLATE ARABIC_CI_AI default '')     
	INSERT INTO #Src (Type , Sec , ReadPrice , UnPostedSec)     
	EXEC prcGetBillsTypesList2 @ReportSource     
	     
	UPDATE src SET OrderName = bt.Abbrev ,   
		       OrderLatinName = bt.LatinAbbrev   
    
	FROM #Src AS src inner join bt000 AS bt ON src.Type = bt.guid      
	...
	CREATE TABLE #CustTbl( Guid UNIQUEIDENTIFIER, [Security] [INT],	CustomerName NVARCHAR(255) collate ARABIC_CI_AI)         
	INSERT INTO [#CustTbl]	(Guid, [Security]) EXEC [prcGetCustsList] @CustGuid,0X00  , @CustCondGuid -- Œÿ«   
						     
	     
	UPDATE C SET CustomerName = cu.CustomerName      
	FROM [#CustTbl] AS C INNER JOIN [CU000] AS [CU] ON [CU].GUID = C.GUID     
...
-------Mat Table----------------------------------------------------------           
	...
	CREATE TABLE #MatTbl(Guid UNIQUEIDENTIFIER, mtSecurity INT,MatName NVARCHAR(800) COLLATE ARABIC_CI_AI default '')             
	INSERT INTO #MatTbl(Guid,mtSecurity)  EXEC prcGetMatsList  @MatGuid, @GroupGuid , -1  , @MatCond            
	-- #matTbl Should be later modified when joining with bi to get right unity and unitfact      
	Update mtTbl Set MatName = Name --+'-'+ Code      
	from #MatTbl as mtTbl inner join mt000 as mt on mt.guid =  mtTbl.guid      
...
-----------------------------------------------------------------------------------------  
	DECLARE @col1 NVARCHAR(100) SET @col1 = dbo.fnGetMatCollectedFieldNameAndGroup(@Collect1, 'mats')   
	DECLARE @col2 NVARCHAR(100) SET @col2 = dbo.fnGetMatCollectedFieldNameAndGroup(@Collect2, 'mats')   
	DECLARE @col3 NVARCHAR(100) SET @col3 = dbo.fnGetMatCollectedFieldNameAndGroup(@Collect3, 'mats')  
----------------------------------------------------------------------------------------- 
...
BEGIN          
	...
	SELECT  cu.CustomerName AS Cust_Name,mt.MatName, bu.Guid OrderGuid, bu.CustGuid ,bi.Guid BiGuid, bi.matguid ,   
		bu.date AS buDate , src.OrderName , src.OrderLatinName,bu.Number as Number , ori.typeGuid,     
		Sum(ori.Qty)AS Qty, bi.Qty AS OrderQty      
	INTO #custQty2     
	FROM bu000 AS bu
	INNER JOIN #CostTbl  ON  #CostTbl.CostGUID=bu.CostGUID      
	...
	INNER JOIN mt000   AS mats ON mt.Guid = mats.GUID   
	INNER JOIN #StoreTbl AS st ON st.guid = bi.storeguid      
	INNER JOIN ori000 AS ori ON ori.poiguid = bi.guid     
	INNER JOIN #TypeTbl AS type ON ori.typeguid = type.Guid        
	...
	GROUP BY bu.Guid, bu.CustGuid ,  cu.CustomerName ,bi.Guid , bi.matguid ,mt.MatName,     
		bu.date  , src.OrderName , src.OrderLatinName ,bu.Number , ori.typeGuid, bi.Qty    
	--ORDER BY (@OrderIndex+1)
	UPDATE #custQty2    
	SET Qty = 0 WHERE Qty < 0     
	-- return first result set      
	SELECT DISTINCT r.typeguid , ISNULL(oit.name,'') as name ,ISNULL(oit.LatinName,'') as LatinName ,oit.postQty    
	FROM #custQty2 AS r LEFT JOIN oit000 AS oit ON  r.typeGuid = oit.guid  order by oit.postQty    
	...
	(case  '+CAST(@useUnit AS NVARCHAR(10))+'   when 1 then ISNULL(r.OrderQty, 0.00)       
	...
	GROUP BY ' 
		...
		+ CASE @Col1 WHEN '' THEN ' BiGuid, MatGuid, MatName , ' ELSE '' END + ' BiGuid, OrderGuid, CustGuid, Cust_Name,buDate , OrderName , OrderLatinName , r.Number ,    
	(case  '+CAST(@useUnit AS NVARCHAR(10))+'   when 1 then mats.Unity       
			when 2 then      
				(case mats.Unit2Fact when 0 then mats.Unity      
					     else mats.Unit2 end )      
	...
	SET @s = ' SELECT DISTINCT r.*, '+ CASE @Col1 WHEN '' THEN ' M.*, ' ELSE '' END + ' C.*, O.* INTO ##temp_____0 
			   FROM ##t0 as r '+
				 CASE @Col1 WHEN '' THEN ' INNER JOIN ##MatFlds M ON M.MatFldGuid = r.MatGuid ' ELSE '' END +
	...
	+CASE @OrderIndex 
			WHEN 1 THEN ' Cust_Name, buDate, MatName   '  
			WHEN 2 THEN ' MatName  '
...
BEGIN      
	...
	SET @ss = 'SELECT '  + @Col1 + (CASE @Col1 WHEN '' THEN '' ELSE ' AS Col1, ' END)  
                         + @Col2 + (CASE @Col2 WHEN '' THEN '' ELSE ' AS Col2, ' END)  
                         + @Col3 + (CASE @Col3 WHEN '' THEN '' ELSE ' AS Col3, ' END)  
                       --+ CASE @Col1 WHEN '' THEN '' ELSE '  ' END +
	+' bu.CustGuid, cu.CustomerName Cust_Name , ori.typeguid , Sum(ori.Qty)  AS Qty     
	INTO ##CustTypeQty     
	FROM bu000 AS bu    
		INNER JOIN #OrderCond OrCond ON  OrCond.OrderGuid = bu.Guid     
		INNER JOIN bi000 AS bi ON bu.guid  = bi.parentguid     
		INNER JOIN #Src AS  src ON src.type =  bu.typeguid      
		INNER JOIN #MatTbl AS mt ON mt.guid = bi.matguid   ' 
		+ CASE @Col1 WHEN '' THEN '' ELSE ' INNER JOIN mt000 AS mats ON mats.Guid = mt.Guid    
		 INNER JOIN gr000 AS gr ON mats.GroupGuid = gr.Guid ' END +
	    'INNER JOIN #StoreTbl AS  st ON st.guid = bi.storeguid       
		INNER JOIN ori000 AS  ori ON ori.poiguid = bi.guid     
	...
	GROUP BY bu.CustGuid, cu.CustomerName , '
		 + @Col1 + (CASE @Col1 WHEN '' THEN '' ELSE ',' END)
		 + @Col2 + (CASE @Col2 WHEN '' THEN '' ELSE ',' END)
		 + @Col3 + (CASE @Col3 WHEN '' THEN '' ELSE ',' END)	
		 + ' ori.typeguid '
	...
	+' bu.CustGuid , bu.Cust_Name , Sum(isnull(bi.Qty,0))AS TotalQty     
	INTO ##CustTotal     
	FROM bu000 AS bu     
	INNER JOIN #OrderCond OrCond ON  OrCond.OrderGuid = bu.Guid    
	INNER JOIN bi000 AS bi ON bu.guid  = bi.parentguid     
	...
	+ CASE @Col1 WHEN '' THEN '' ELSE ' INNER JOIN mt000 AS mats ON mats.Guid = mt.Guid 
		 INNER JOIN gr000 AS gr ON mats.GroupGuid = gr.Guid' END +
	' INNER JOIN ORADDINFO000 OInfo ON bu.Guid = OInfo.ParentGuid   
	WHERE   bu.date BETWEEN '''+CAST(@StartDate AS NVARCHAR(50))+''' AND '''+CAST(@EndDate AS NVARCHAR(50))+'''	      
	AND  (OInfo.Finished =( Case '+CAST(@IsFinished AS NVARCHAR(50))+' WHEN 0 THEN 0 else OInfo.Finished end  ) )   
		AND (OInfo.Add1 =( Case '+CAST(@IsCancled AS NVARCHAR(50))+' WHEN 0 THEN ''0'' else OInfo.Add1 end  ) )   
	GROUP BY bu.CustGuid, bu.Cust_Name '
			+ (CASE @Col1 WHEN '' THEN '' ELSE ', ' END)  + @Col1
		    + (CASE @Col2 WHEN '' THEN '' ELSE ', ' END)  + @Col2
			+ (CASE @Col3 WHEN '' THEN '' ELSE ', ' END)  + @Col3
	EXEC (@ss)
	 -----------------------------------------------------------------------------------------------------------------------   
	 
	EXEC (' SELECT DISTINCT r.typeguid , oit.name as name ,oit.LatinName as LatinName ,oit.postQty    
	from ##CustTypeQty AS r LEFT JOIN oit000 AS oit ON  r.typeguid  = oit.guid order by oit.postQty ')
		 
	SET @ss = 'SELECT ' + (CASE @Col1 WHEN '' THEN '' ELSE 'r.Col1' END) + (CASE @Col1 WHEN '' THEN '' ELSE ', ' END) 
                        + (CASE @Col2 WHEN '' THEN '' ELSE 'r.Col2' END) + (CASE @Col2 WHEN '' THEN '' ELSE ', ' END) 
                        + (CASE @Col3 WHEN '' THEN '' ELSE 'r.Col3' END) + (CASE @Col3 WHEN '' THEN '' ELSE ', ' END)  
                        
	+' r.CustGuid , r.Cust_Name , t1.TotalQty, r.typeguid , ISNULL(r.Qty, 0.00) AS Qty, C.*  
	FROM ##CustTypeQty AS r     
	INNER JOIN  ##CustTotal AS t1 ON t1.CustGuid = r.CustGuid '
	+ CASE @Col1 WHEN '' THEN '' ELSE ' AND r.Col1 = t1.Col1 ' END
	+ CASE @Col2 WHEN '' THEN '' ELSE ' AND r.Col2 = t1.Col2 ' END
	+ CASE @Col3 WHEN '' THEN '' ELSE ' AND r.Col3 = t1.Col3 ' END
...
BEGIN      
	...
	SET @str = 'SELECT ' + @Col1 + (CASE @Col1 WHEN '' THEN '' ELSE ' AS Col1, ' END)  
                         + @Col2 + (CASE @Col2 WHEN '' THEN '' ELSE ' AS Col2, ' END)  
                         + @Col3 + (CASE @Col3 WHEN '' THEN '' ELSE ' AS Col3, ' END)  
                       
	+' bu.CustGuid, bu.guid as orderguid, cu.CustomerName Cust_Name , ori.typeguid , Sum(ori.Qty)  AS Qty     
	...
	WHERE   ori.date BETWEEN '''+CAST(@StartDate AS NVARCHAR(50))+''' AND '''+CAST(@EndDate AS NVARCHAR(50))+'''   
		AND  (OInfo.Finished =( Case '+CAST(@IsFinished AS NVARCHAR(50))+' WHEN 0 THEN 0 else OInfo.Finished end  ) )   
		AND (OInfo.Add1 =( Case '+CAST(@IsCancled AS NVARCHAR(50))+' WHEN 0 THEN ''0'' else OInfo.Add1 end  ) )   
	GROUP BY bu.CustGuid, bu.guid, cu.CustomerName , '
		  + @Col1 + (CASE @Col1 WHEN '' THEN '' ELSE ',' END)
		 + @Col2 + (CASE @Col2 WHEN '' THEN '' ELSE ',' END)
		  + @Col3 + (CASE @Col3 WHEN '' THEN '' ELSE ',' END)	
		 + ' ori.typeguid '
	...
	+' bu.CustGuid , bu.guid as orderguid, bu.date as budate, src.ordername, bu.number as ordernumber, bu.Cust_Name , Sum(isnull(bi.Qty,0))AS TotalQty     
	INTO ##CustOrderTotal     
	FROM bu000 AS bu     
	INNER JOIN #OrderCond OrCond ON  OrCond.OrderGuid = bu.Guid    
	INNER JOIN bi000 AS bi ON bu.guid  = bi.parentguid     
	...
	AND  (OInfo.Finished =( Case '+CAST(@IsFinished AS NVARCHAR(50))+' WHEN 0 THEN 0 else OInfo.Finished end  ) )   
		AND (OInfo.Add1 =( Case '+CAST(@IsCancled AS NVARCHAR(50))+' WHEN 0 THEN ''0'' else OInfo.Add1 end  ) )   
	GROUP BY bu.CustGuid, bu.guid, bu.date, src.ordername, bu.number, bu.Cust_Name '
			+ (CASE @Col1 WHEN '' THEN '' ELSE ', ' END)  + @Col1
		    + (CASE @Col2 WHEN '' THEN '' ELSE ', ' END)  + @Col2
			+ (CASE @Col3 WHEN '' THEN '' ELSE ', ' END)  + @Col3    
	EXEC (@str)
	...
	EXEC (' SELECT DISTINCT r.typeguid , oit.name as name ,oit.LatinName as LatinName ,oit.postQty    
	from ##CustOrderTypeQty AS r LEFT JOIN oit000 AS oit ON  r.typeguid  = oit.guid order by oit.postQty ')
		 
	SET @str = 'SELECT ' + (CASE @Col1 WHEN '' THEN '' ELSE 'r.Col1' END) + (CASE @Col1 WHEN '' THEN '' ELSE ', ' END) 
                        + (CASE @Col2 WHEN '' THEN '' ELSE 'r.Col2' END) + (CASE @Col2 WHEN '' THEN '' ELSE ', ' END) 
                        + (CASE @Col3 WHEN '' THEN '' ELSE 'r.Col3' END) + (CASE @Col3 WHEN '' THEN '' ELSE ', ' END)  
                        
	+' r.CustGuid , r.orderguid, t1.ordername, t1.budate, t1.ordernumber, r.Cust_Name , t1.TotalQty, r.typeguid , ISNULL(r.Qty, 0.00) AS Qty, O.*, C.*  
	FROM ##CustOrderTypeQty AS r     
	INNER JOIN  ##CustOrderTotal AS t1 ON t1.CustGuid = r.CustGuid AND t1.orderguid = r.orderguid '
	+ CASE @Col1 WHEN '' THEN '' ELSE ' AND r.Col1 = t1.Col1 ' END
	+ CASE @Col2 WHEN '' THEN '' ELSE ' AND r.Col2 = t1.Col2 ' END
	+ CASE @Col3 WHEN '' THEN '' ELSE ' AND r.Col3 = t1.Col3 ' END
	+ ' LEFT JOIN ##CustFlds C ON C.CustFldGuid = r.CustGuid '
