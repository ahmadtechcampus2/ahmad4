###########################
CREATE  FUNCTION fnGetRadioGraphyListSorted( 
...
AS BEGIN 
	DECLARE @FatherBuf TABLE( GUID UNIQUEIDENTIFIER, [Level] INT, [Path] NVARCHAR(max), ID INT IDENTITY( 1, 1)) 
	DECLARE @Continue INT, @Level INT 
	SET @Level = 0      
	  
	IF ISNULL( @RadioGraphyGUID, 0x0) = 0x0 
		INSERT INTO @FatherBuf ( GUID, Level, [Path])   
			SELECT GUID, @Level, ''  
			FROM 	 vwHosRadioGraphyAll 
			WHERE  ISNULL( ParentGUID, 0x0) = 0x0  
			ORDER BY CASE @Sorted WHEN 1 THEN Code ELSE [Name] END  
	ELSE    
		INSERT INTO @FatherBuf ( GUID, Level, [Path])   
			SELECT GUID, @Level, '' FROM vwHosRadioGraphyAll WHERE GUID = @RadioGraphyGUID 
	...
	BEGIN    
		...
		INSERT INTO @FatherBuf( GUID, Level, [Path])    
			SELECT Ana.GUID, @Level, fb.[Path]   
				FROM vwHosRadioGraphyAll AS Ana INNER JOIN @FatherBuf AS fb ON Ana.ParentGUID = fb.GUID 
