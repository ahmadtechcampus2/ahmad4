...
FOR SELECT MnID, Qty FROM @ReadyItems 
OPEN mnCursor 
...
BEGIN 
	...
	INSERT INTO #RowMaterials EXEC prcGetManufacMaterialTree @MatID, '', '', 0x0, @QTY
...
FOR SELECT MatGUID, NeededFormsCountTemp, QtyInForm FROM #RowMaterials WHERE [IsSemiReadyMat] = 1
OPEN rowMaterialCursor 
FETCH FROM rowMaterialCursor INTO @MatID, @QTY, @QtyInForm
WHILE @@FETCH_STATUS = 0 
BEGIN 
	SELECT @MnGUID = [ParentGuid] FROM mi000 WHERE MATGUID =  @MatID AND TYPE = 0
	INSERT INTO @FinalReadyItems VALUES(@MnGUID, @QTY, 0)
	
	FETCH NEXT FROM rowMaterialCursor INTO @MatID, @QTY, @QtyInForm
END
CLOSE rowMaterialCursor 
DEALLOCATE rowMaterialCursor 
-----------------------------------------------------------------------------

INSERT INTO @FinalReadyItems SELECT MnID, SUM(Qty) / (SELECT [Qty] FROM mi000 WHERE ParentGUID =  MnID AND TYPE = 0), 1 FROM @ReadyItems GROUP BY MnID
...
BEGIN
	...
	-- this operation to exclude the hold items which added in restaurant (type = 4 is hold items)
	INSERT INTO mi000([Type],[Number],[Unity],[Qty],[Notes],[CurrencyVal],[Price]
      ...
      ,[CostGUID],[Percentage])
	SELECT [Type],[Number],[Unity],([Qty] * @Qty) - (SELECT ISNULL(SUM(items.Qty),0) FROM RestOrder000 orders INNER JOIN RestOrderItem000 items ON items.ParentID=orders.Guid  WHERE items.Type=4 AND items.MatID = [MatGUID] AND orders.Period=@Period) ,[Notes],[CurrencyVal],[Price]
      ,[Class], newid() /*[GUID]*/ ,[Qty2],[Qty3], @GUID /*[ParentGUID]*/,[MatGUID],[StoreGUID]
      ,[CurrencyGUID],[ExpireDate],[ProductionDate],[Length],[Width],[Height]
      ,[CostGUID],[Percentage]
	FROM [MI000] 
	WHERE ParentGUID = @MnGUID
	--AND (([Qty] * @Qty) - (SELECT ISNULL(SUM(items.Qty),0) FROM RestOrder000 orders INNER JOIN RestOrderItem000 items ON items.ParentID=orders.Guid  WHERE items.Type=4 AND items.MatID = [MatGUID] AND orders.Period=@Period)) > 0
