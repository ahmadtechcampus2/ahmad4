##########################################################
CREATE PROCEDURE prcGetTableIndexes
	@TableID BIGINT
AS
	SET NOCOUNT ON
	DECLARE
	...
	BEGIN
		...
		INNER JOIN #stringColumns c ON ic.object_id = @TableID and ic.column_id = c.ColumnID
		...
		INNER JOIN sys.objects so ON so.object_id=si.object_id AND so.object_id = @TableID
	...
	--SET @sSQL = 'IF NOT EXISTS (SELECT * FROM sysobjects WHERE name = ''' + @idxname + ''') '
	--SET @sSQLDrop = 'IF EXISTS (SELECT * FROM sysobjects WHERE name = ''' + @idxname + ''') '
	...
	IF (@IsPrimaryKey = 1)
	BEGIN
	 SET @sSQL = @sSQL + 'ALTER TABLE ['+@IdxSchema+'].[' + @idxTableName + '] ADD CONSTRAINT [' + @idxname + '] PRIMARY KEY '
	 SET @sSQLDrop = @sSQLDrop + 'ALTER TABLE ['+@IdxSchema+'].[' + @idxTableName + '] DROP CONSTRAINT [' + @idxname + ']'
	...
	BEGIN
	 ...
	 SET @sSQLDrop = @sSQLDrop + 'DROP INDEX [' + @idxname + '] ON ['+@IdxSchema+'].[' + @idxTableName + ']'
	 -- Check if the index is unique
	...
	SELECT DISTINCT TableName, IndexName, IsClustered, IsPrimaryKey, IndexCreateSQL, IndexDropSQL FROM #IndexSQL ORDER BY 1,2 --WHERE IsPrimaryKey=0
#########################################################
#END